// Unit test for gadgets/dnf_distribute.h, run entirely in the clear via
...
void check(ClearSession& sess, const char* name,
    ...
    if (bits != bits_of(expect_bits) || mask != bits_of(expect_mask) || pad != expect_pad) {
...
int run_dnf_distribute_tests() {
    ...
    std::array<CircuitCube<Ctx, N>, 2> d1 = {make_cube(sess, "1000", "1000"),   // x1
                                              make_cube(sess, "0100", "0100")}; // x2
    std::array<CircuitCube<Ctx, N>, 2> d2 = {make_cube(sess, "1000", "1000"),   // x1 (agrees with d1[0])
                                              make_cube(sess, "0000", "1000")}; // ~x1 (conflicts with d1[0])
    ...
    assert(sess.reveal(cross[0].bits, PUBLIC).value() == bits_of("1000"));
    // d1[0] /\ d2[1]: x1 /\ ~x1 -> contradiction -> padding
    assert(sess.reveal(cross[1].pad, PUBLIC).value());
    assert(sess.reveal(cross[1].bits, PUBLIC).value() == bits_of("1111"));
    assert(sess.reveal(cross[1].mask, PUBLIC).value() == bits_of("0000"));
    // d1[1] /\ d2[0]: x2 /\ x1 -> ordinary, disjoint, bits=1100
    assert(!sess.reveal(cross[2].pad, PUBLIC).value());
    assert(sess.reveal(cross[2].bits, PUBLIC).value() == bits_of("1100"));
    // d1[1] /\ d2[1]: x2 /\ ~x1 -> ordinary, disjoint, bits=0100
    assert(!sess.reveal(cross[3].pad, PUBLIC).value());
    assert(sess.reveal(cross[3].bits, PUBLIC).value() == bits_of("0100"));
